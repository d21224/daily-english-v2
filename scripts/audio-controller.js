import { parseSegment } from './rules.js?v=0.2.2';
...
export class AudioController {
  ...
  async start(day) {
    ...
    this.active = { day, segment, runId: crypto.randomUUID(), reachedFromStart: true };
  ...
  update() {
    ...
    const percent = Math.max(0, Math.min(100, ((this.player.currentTime - start) / (end - start)) * 100));
    this.callbacks.onProgress?.({ percent, paused: this.player.paused, dayId: this.active.day.id });
    if (this.player.currentTime >= end - .03) this.complete();
  }
