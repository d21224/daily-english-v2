...
import { calculateReward, getProgress, hasActiveListening, mondayKey, parseSegment, rewardCopy, totalPoints, validateState } from '../scripts/rules.js';
...
test('목표가 1 이상인 듣기만 오디오를 요구한다', () => {
  const state = createDefaultState();
  state.weeklyTasks = [];
  assert.equal(hasActiveListening(state), false);
  state.days[2].target = 1;
  state.days[2].segment = '0:00-0:10';
  assert.equal(hasActiveListening(state), true);
});
